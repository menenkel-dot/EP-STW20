  ...
  return (
    <header className="flex items-center justify-between h-20 px-6 bg-white border-b-2 border-gray-200 dark:bg-gray-800 dark:border-gray-700">
      <div className="flex items-center">
        <button onClick={onMenuClick} className="md:hidden text-gray-500 focus:outline-none focus:text-gray-700 mr-4 dark:text-gray-400 dark:focus:text-gray-200">
        ...
        <div className="relative ml-4">
          ...
          >
            <span className="hidden md:inline-block mr-3 text-right">
                ...
                <br/>
                <span className="text-sm text-gray-500 dark:text-gray-400">{activeChild ? `Kind: ${activeChild.name}` : user.role === 'admin' ? 'Verwaltung' : ''}</span>
            </span>
          ...
          {isProfileMenuOpen && (
            <div 
              ...
              className="absolute right-0 mt-2 w-56 bg-white rounded-md shadow-xl z-20 dark:bg-gray-700"
              role="menu"
            >
              <a
                href="#"
                ...
                className="block px-4 py-2 text-sm text-gray-700 hover:bg-cyan-500 hover:text-white dark:text-gray-200 dark:hover:bg-cyan-600"
                role="menuitem"
              >
                <div className="flex items-center">
              ...
              <a
                ...
                className="block px-4 py-2 text-sm text-gray-700 hover:bg-cyan-500 hover:text-white rounded-b-md dark:text-gray-200 dark:hover:bg-cyan-600"
                role="menuitem"
              >
                Abmelden
              </a>
        ...
        {hasMultipleChildren && (
            <div className="relative ml-2">
                <button 
                  ...
                  className="p-2 rounded-full hover:bg-gray-100 focus:outline-none dark:hover:bg-gray-700"
                ...
                {isChildMenuOpen && (
                    <div 
                      ...
                      role="menu"
                    >
                        <div className="py-2 px-4 text-gray-700 font-semibold bg-gray-50 dark:bg-gray-600 dark:text-gray-200">Kind wechseln</div>
                        {user.children.map(child => (
                            <a
                                ...
                                className={`flex items-center px-4 py-3 hover:bg-gray-100 transition-colors duration-150 ${activeChild?.id === child.id ? 'bg-cyan-50 dark:bg-cyan-900/50' : ''} dark:hover:bg-gray-600`}
                                role="menuitem"
                            >
                                ...
                                <span className={`text-sm ${activeChild?.id === child.id ? 'font-semibold text-cyan-800 dark:text-cyan-100' : 'text-gray-700 dark:text-gray-200'}`}>{child.name}</span>
                            </a>
                        ))}
                    </div>
                )}
